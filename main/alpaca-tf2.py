...
class ALPaCA:
    ...
    def construct_model(self):
        with self.graph.as_default():
            ...
            self.optimizer = AdamW(weight_decay=0.3, learning_rate=self.lr)

            global_step = tf.Variable(0, trainable=False, name='global_step')
            self.train_op = self.optimizer.minimize(self.total_loss, tf.trainable_variables)#global_step=global_step)
        ...
        for i in range(num_train_updates):
            ...
            if i % 1000 == 0:
                if valid_dataset is not None:
                    ...
                    print('Loss: %.3f - Valid-LL: %.3f - Valid-RMSE: %.3f - Calib-Err %.3f ' % (loss, alpaca_ll, alpaca_rmse, alpaca_calib))
                else:
                    print('loss:', loss)
